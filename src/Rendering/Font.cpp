...
void Font::load(const std::string& fontFileName, int fontSize)
{
	initFreeType();

	App::getLog().logInfo("Loading font from %s (size: %s)", fontFileName, fontSize);
...
void Font::initFreeType()
{
	if (library == nullptr)
	{
		App::getLog().logInfo("Initializing FreeType library");
...
Font::Glyph& Font::getGlyph(char32_t characterCode)
{
	if (glyphs.count(characterCode) == 0)
	{
		...
		if (error)
			throw std::runtime_error(tfm::format("Could not load the font character (code: %s): %s", characterCode, error));
