...
Color WoodTexture::getColor(const Vector3& position, const Vector2& texcoord) const
{
	...
	double n1 = perlinNoise.getOctaveNoise(8, 0.3, position.x * 2.0, position.y * 0.1, position.z * 2.0);
	n1 *= 8.0;
	n1 -= (int)n1;
	n1 += 0.4;
	n1 = std::min(n1, 1.0);
	...
	// subtle streaks
	double n3 = perlinNoise.getOctaveNoise(4, 0.01, position.x * 200.0, position.y * 1.0, position.z * 2.0);
	n3 = 1.0 - n3 * n3 * n3 * n3;
	n3 = 0.75 + n3 / 4.0;
